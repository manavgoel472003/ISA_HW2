...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--nus-cams", default="all",
        help="Comma-separated camera names to use for NuScenes custom loader ('all' for default six views)"
    )
    ap.add_argument(
        "--ann-file", default="",
        help=(
    ...
    ap.add_argument(
        ...
        help="Global seed applied to Python/NumPy/Torch (-1 disables seeding)"
    )
...
def main() -> None:
    ...
    if args.eval and args.eval_backend == "runner":
        ...
        run_benchmark_evaluation(args, sys_info, summary_meta=summary_meta)
        return
    ...
    if args.nus_cams.lower() == "all":
        nus_cam_list = None
    else:
        nus_cam_list = [c.strip() for c in args.nus_cams.split(',') if c.strip()]
    ...
    if args.eval and args.eval_backend == "manual":
        ...
        eval_set = "val"
        if args.dataset == "nuscenes" and "mini" in args.nus_version:
            eval_set = "mini_val"
        ...
        run_manual_benchmark(
            ...
            device=args.device,
            eval_set=eval_set,                 # Auto-switch for v1.0-mini
