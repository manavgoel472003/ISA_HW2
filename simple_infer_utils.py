...
from mmengine.utils import set_random_seed as mmengine_set_seed
...
def _update_experiment_summary(out_dir: str, entry: Dict[str, Any]) -> None:
    """Write or update the global all_experiments_summary.json file."""
    try:
        out_path = Path(out_dir).resolve()
    except Exception:
        out_path = Path(out_dir)

    summary_path = out_path.parent / "all_experiments_summary.json"
...
def set_all_seeds(seed: Optional[int]) -> None:
    """
    Seed Python, NumPy, PyTorch, and MMEngine so inference is reproducible.

    Args:
        seed: integer seed (ignored if None or < 0)
    """
    ...
    try:
        mmengine_set_seed(seed, deterministic=False)
    except Exception:
        pass
...
def boxes_to_lineset(boxes: Optional[np.ndarray],
    ...
    for i, b in enumerate(box_arr):
        ...
        lines.extend([[base + u, base + v] for u, v in BOX_EDGE_IDX])
...
def _boxes_to_sampled_points(
    ...
    """
    Convert a handful of boxes into point samples along their edges.

    This lets us serialize wireframes into regular point clouds (PLY) so that
    lightweight viewers still display box outlines.
    """
    if boxes is None or len(boxes) == 0:
        return None
    try:
    ...
    points, colors = [], []
    limit = min(len(box_arr), max_boxes)
    linspace = np.linspace(0.0, 1.0, samples_per_edge, dtype=np.float32)

    for b in box_arr[:limit]:
        if b.shape[0] < 7:
            continue
        x, y, z, dx, dy, dz, yaw = b[:7]
        c, s = math.cos(yaw), math.sin(yaw)
        ...
        yc = [dy/2, -dy/2, -dy/2, dy/2, dy/2, -dy/2, -dy/2, dy/2]
        zc = [0, 0, 0, 0, dz, dz, dz, dz]
        corners = (R @ np.vstack([xc, yc, zc])).T + np.array([x, y, z], dtype=np.float32)

        for u, v in BOX_EDGE_IDX:
            start = corners[u]
            end = corners[v]
            edge = (end - start)[None, :]
            samples = start[None, :] + linspace[:, None] * edge
            points.append(samples)
            colors.append(np.tile(color_arr, (samples_per_edge, 1)))

    if not points:
        return None

    pts_arr = np.concatenate(points, axis=0)
    col_arr = np.concatenate(colors, axis=0)
...
def save_ply_files(out_dir: str,
    ...
    # Also emit a single point cloud that includes wireframe samples so Open3D
    # viewers that only load point clouds will still show up to three boxes.
    enriched = _boxes_to_sampled_points(pred_boxes, color=[0, 1, 0])
    gt_enriched = _boxes_to_sampled_points(gt_boxes, color=[1, 0, 0])

    extra_pts, extra_cols = [], []
    if enriched is not None:
        extra_pts.append(enriched[0])
        extra_cols.append(enriched[1])
    if gt_enriched is not None:
        extra_pts.append(gt_enriched[0])
        extra_cols.append(gt_enriched[1])

    if extra_pts:
        base_points = pts[:, :3]
        base_colors = np.full((base_points.shape[0], 3), 0.5, dtype=np.float32)
        extra_points = np.concatenate(extra_pts, axis=0)
        extra_colors = np.concatenate(extra_cols, axis=0)

        combo = o3d.geometry.PointCloud()
        combo.points = o3d.utility.Vector3dVector(
            np.concatenate([base_points, extra_points], axis=0)
        )
        combo.colors = o3d.utility.Vector3dVector(
            np.concatenate([base_colors, extra_colors], axis=0)
        )
        combo_path = osp.join(out_dir, f"{token}_points_with_boxes.ply")
        o3d.io.write_point_cloud(combo_path, combo)
...
def draw_2d_multiview(paths: List[str],
    ...
    cv2.imwrite(osp.join(out_dir, f"{token}_multiview.png"), final)


def draw_bev_projection(pts: np.ndarray,
                        pred_boxes: Optional[np.ndarray],
                        token: str,
                        out_dir: str) -> None:
    """Render BEV scatter plot with optional boxes when RGB views unavailable."""
    try:
        import matplotlib
        matplotlib.use('Agg')
    ...
    sc = ax.scatter(pts[:, 0], pts[:, 1], s=0.3, c=pts[:, 2],
    ...
    if pred_boxes is not None and len(pred_boxes) > 0:
        for box in pred_boxes:
            ...
            pts_xy = (rot @ corners.T).T + np.array([x, y], dtype=np.float32)
            poly = plt.Polygon(pts_xy, fill=False, edgecolor='red',
                               linewidth=1.0)
            ax.add_patch(poly)
    ...
    ax.set_xlabel('X (m)')
    ax.set_ylabel('Y (m)')
    ax.set_title(f"BEV Projection - {token}")
    ax.set_xlim(PC_RANGE_DEFAULT[0], PC_RANGE_DEFAULT[3])
    ax.set_ylim(PC_RANGE_DEFAULT[1], PC_RANGE_DEFAULT[4])
    fig.tight_layout()
    fig.savefig(osp.join(out_dir, f"{token}_bev.png"), dpi=200)
    plt.close(fig)
...
def draw_2d_multiview_from_tensor(
    ...
    Args:
        ...
        out_dir:     directory to write <token>_multiview.png.
    """
...
class NuScenesLoader(BaseLoader):
    ...
    def __init__(self,
        ...
        self.cam_list = cam_list if cam_list else NUSC_CAMS
    ...
    def load_points(self, sample: Dict[str, Any]) -> np.ndarray:
        ...
        for i in range(self.nsweeps):
            ...
            # Match MMDet3D's LoadPointsFromMultiSweeps convention where
            # channel 4 stores the time-lag relative to the reference sweep.
            ref_ts = ref_sd['timestamp']
            curr_ts = curr_sd['timestamp']
            time_lag = float(ref_ts - curr_ts) / 1e6
            pts[:, 4] = time_lag
    ...
    def load_imgs(self, sample: Dict[str, Any],
        ...
        ori_shapes: List[Tuple[int, int, int]] = []

        for c_name in self.cam_list:
        ...
        aug_matrix = np.tile(
            np.eye(4, dtype=np.float32)[None], (len(self.cam_list), 1, 1))

        # NOTE:
        #   - `scale_factor` stays 2D (`[w_scale, h_scale]`) because FCOS3D
        #     divides its xy offsets by the first two elements and asserts on
        #     that shape.
        #   - `batch_input_shape` is expected by dense heads when building
        #     anchor grids.
        lidar2img_arr = np.stack(lidar2img)
        cam2img_arr = np.stack(cam2img)
        lidar2cam_arr = np.stack(lidar2cam)
        cam2lidar_arr = np.linalg.inv(lidar2cam_arr)

        metainfo = {
            'lidar2img': lidar2img_arr,
            'cam2img': cam2img_arr if len(self.cam_list) > 1 else cam2img_arr[0],
            'lidar2cam': lidar2cam_arr,
            'cam2lidar': cam2lidar_arr,
            'img_aug_matrix': aug_matrix,
            'img_shape': [target_size] * len(self.cam_list),
            'box_type_3d': LiDARInstance3DBoxes,
            'ori_shape': ori_shapes,
            'pad_shape': [tuple(target_size) + (3, )] * len(self.cam_list),
            'scale_factor': np.array([scale, scale], dtype=np.float32),
            'flip': False,
            'flip_direction': None,
            'batch_input_shape': tuple(target_size),
            'img_norm_cfg': {
                'mean': [0.0, 0.0, 0.0],
                'std': [1.0, 1.0, 1.0],
                'to_rgb': False
...
class KittiSequenceLoader(BaseLoader):
    """Simple loader for KITTI odometry-style sequences (velodyne only)."""
    ...
    def __init__(self,
        ...
        self.dataroot = dataroot
        seq_root = osp.join(dataroot, 'sequences')
        if not osp.isdir(seq_root):
            raise FileNotFoundError(f"Expected sequences folder at {seq_root}")

        seq_candidates = sorted(
            d for d in os.listdir(seq_root)
            if osp.isdir(osp.join(seq_root, d)))
        if sequences:
            seq_candidates = [s for s in seq_candidates if s in sequences]

        self.samples = []
        for seq in seq_candidates:
            velo_dir = osp.join(seq_root, seq, 'velodyne')
            frame_paths = sorted(glob.glob(osp.join(velo_dir, '*.bin')))
            for idx, path in enumerate(frame_paths):
                token = f"{seq}_{idx:06d}"
                self.samples.append({
                    'token': token,
                    'seq': seq,
                    'frame': idx,
    ...
    def __getitem__(self, idx: int) -> Dict[str, Any]:
        sample = self.samples[idx]
        pts = np.fromfile(sample['velodyne'], dtype=np.float32).reshape(-1, 4)
        meta = {
            'sample_idx': sample['frame'],
            'token': sample['token'],
            'sequence': sample['seq'],
            'lidar2img': [],
            'box_type_3d': LiDARInstance3DBoxes,
...
def _infer_point_feature_dim(cfg: Config) -> Optional[int]:
    """
    Inspect the config to determine how many point features the model expects.

    Returns:
        Integer number of channels if available, else None.
    """
    model_cfg = getattr(cfg, "model", None)
    if isinstance(model_cfg, dict):
        voxel_encoder = model_cfg.get("pts_voxel_encoder")
        if isinstance(voxel_encoder, dict):
            in_channels = voxel_encoder.get("in_channels")
...
def build_loader_pack(
    ...
    nus_cams: Optional[List[str]] = None,
    ann_file: str = "",
    max_samples: int = -1,
    ...
    Args:
        ...
        nus_cams:    Optional list of NuScenes camera names for custom loader
    ...
    """
    point_dim = _infer_point_feature_dim(cfg)
    ...
    if dataset == "nuscenes":
        ...
        return dict(
            loader=loader,
            iter_fn=custom_iter,
            nusc=dataset_obj.nusc,
            dataset=dataset_obj,
            point_dim=point_dim,
        )

    if dataset == "kitti":
        dataset_obj = KittiSequenceLoader(
            dataroot=dataroot,
            max_samples=max_samples,
        )
        loader = DataLoader(
    ...
    raise NotImplementedError(
        "Custom raw loader is only implemented for NuScenes and KITTI."
    )
# -----------------------------------------------------------------------------
...
def run_benchmark_evaluation(args, sys_info: Dict[str, Any],
                             summary_meta: Optional[Dict[str, Any]] = None) -> None:
    """
    ...
    if summary_meta:
        entry = dict(summary_meta)
        entry.update({
            "dataset": args.dataset,
            "mode": "runner",
            "benchmark_file": json_path,
            "metrics": final_metrics,
            "performance": perf_stats,
        })
        _update_experiment_summary(args.out_dir, entry)
...
def _ensure_point_dim(pts: Any, point_dim: Optional[int]) -> Any:
    """
    Align point feature dimension with what the model expects.

    If the points array has more channels than required, extra channels
    are truncated. If it has fewer, zeros are appended.
    """
    if point_dim is None or pts is None:
        return pts
    ...
    if pts.shape[1] == point_dim:
        return pts
    if pts.shape[1] > point_dim:
        return pts[:, :point_dim]
    pad = np.zeros((pts.shape[0], point_dim - pts.shape[1]), dtype=pts.dtype)
    return np.concatenate([pts, pad], axis=1)
...
def _maybe_save_multiview(
    ...
    if 'img' not in inputs or inputs['img'] is None:
        draw_bev_projection(pts_np[:, :3], pred_boxes_np, token, out_dir)
        return
...
def run_manual_benchmark(
    ...
    if dataset.lower() == "kitti":
        ...
        for token, pts, imgs, meta, gt_boxes, gt_labels in pbar:
            ...
            pts = _ensure_point_dim(pts, point_dim)
        ...
        if summary_meta:
            ...
            _update_experiment_summary(out_dir, entry)
        return
    ...
    for loader_token, pts, imgs, meta, gt_boxes, gt_labels in pbar:
        ...
        save_token = sample_token or str(loader_token)
        save_ply_files(
            out_dir=out_dir,
            token=str(save_token),
            pts=pts_np,
            pred_boxes=pred_boxes_np,
            gt_boxes=gt_boxes_np,
        )
    ...
    if summary_meta:
        if hasattr(metrics_summary, "serialize"):
            try:
                metrics_payload = metrics_summary.serialize()
            except Exception:
                metrics_payload = {
                    "NDS": float(metrics_summary.nd_score),
                    "mAP": float(metrics_summary.mean_ap),
        ...
        entry.update({
            ...
            "benchmark_file": osp.join(out_dir, "benchmark_nuscenes.json"),
...
def inference_loop(model: torch.nn.Module,
    ...
    Args:
        ...
        save_images: if True, save multiview PNGs
    ...
    for idx, (token, pts, imgs, meta, paths, _) in pbar:
        ...
        if imgs is not None:
            imgs_tensor = imgs.to(device_obj)
            if imgs_tensor.dim() == 4 and imgs_tensor.shape[0] == 1:
